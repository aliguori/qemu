...
static void virtio_vga_map_regions(VirtIOPCIProxy *vdev)
{
    /* Do nothing.  We'll handle it in our realize function */
}
...
static int virtio_vga_realize(VirtIOPCIProxy *vdev)
{
    ...
    ret = qdev_init(DEVICE(&s->fb));
    if (ret) {
        return ret;
    }

    vga_common_init_ext(vga, &s->fb.vram, s->fb.vram_size);

    vga_init(vga, pci_address_space(pcidev), pci_address_space_io(pcidev), true);
    vga->con = s->fb.con;


    pci_register_bar(pcidev, 0, PCI_BASE_ADDRESS_MEM_PREFETCH, vga->vram);
    ...
    memory_region_add_subregion(&s->mmio, 0, &vdev->bar);
...
static void virtio_vga_invalidate(VirtioFB *fb)
{
    VirtioVGA *s = container_of(fb, VirtioVGA, fb);
    s->vga.hw_ops->invalidate(&s->vga);
}

static void virtio_vga_gfx_update(VirtioFB *fb)
{
    VirtioVGA *s = container_of(fb, VirtioVGA, fb);
    s->vga.hw_ops->gfx_update(&s->vga);
}

static void virtio_vga_text_update(VirtioFB *fb, console_ch_t *chardata)
{
    VirtioVGA *s = container_of(fb, VirtioVGA, fb);
    s->vga.hw_ops->text_update(&s->vga, chardata);
}

static const GraphicHwOps virtio_vga_hw_ops = {
    .invalidate = (GraphicHwInvalidate *)virtio_vga_invalidate,
    .gfx_update = (GraphicHwGFXUpdate *)virtio_vga_gfx_update,
    .text_update = (GraphicHwTextUpdate *)virtio_vga_text_update,
};

static void virtio_vga_initfn(Object *obj)
{
    VirtioVGA *s = VIRTIO_VGA(obj);

    object_initialize(OBJECT(&s->fb), TYPE_VIRTIO_FB);
    object_property_add_child(obj, "virtio-backend", OBJECT(&s->fb), NULL);
    s->fb.hw_ops = &virtio_vga_hw_ops;
}
...
static void virtio_vga_class_init(ObjectClass *klass, void *data)
{
    ...
    vpc->map_regions = virtio_vga_map_regions;
}
...
static TypeInfo virtio_vga_info = {
    ...
    .instance_init = virtio_vga_initfn,
