...
{
    Wire *s = container_of(notifier, Wire, in_level_change);

    pin_set_level(s->out, pin_get_level(s->in));
}
...
void wire_realize(Wire *s)
{
    ...
    s->in_level_change.notify = wire_notify_update;
    s->in_level_change.release = wire_notify_release;
    pin_add_level_change_notifier(s->in, &s->in_level_change);
    s->connected = true;
}
...
static void wire_finalize(Object *obj)
{
    ...
    if (s->connected) {
        pin_del_level_change_notifier(s->in, &s->in_level_change);
        s->connected = false;
    }
}
